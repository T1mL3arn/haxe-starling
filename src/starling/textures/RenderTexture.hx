...
class RenderTexture extends SubTexture
{
                                  ...
                                  scale:Float=-1, format:Context3DTextureFormat=null, repeat:Bool=false)
    {
        ...
        {
            legalWidth  = StarlingUtils.getNextPowerOfTwo(Std.int(width  * scale)) / scale;
            legalHeight = StarlingUtils.getNextPowerOfTwo(Std.int(height * scale)) / scale;
        }
    ...
    private function renderBundled(renderBlock:Dynamic, object:DisplayObject=null,
                                   ...
                                   antiAliasing:Int=0):Void
    {
        var context:Context3D = cast Starling.current.context;
